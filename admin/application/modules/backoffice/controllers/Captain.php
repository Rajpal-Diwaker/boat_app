<?php
//Captain controller to manage website queries
class Captain extends MX_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('backoffice/Captainmodel');
	...
	function listing(){
        ...
        $data['page_title'] = 'Captain Listing';
        $config['base_url'] = base_url('backoffice/Captain/listing');
        ...
        $data['resultArr'] = $this->Captainmodel->listing($config["per_page"], $data['page'], $data['searchFor'], $data['orderField'], $data['orderDirection']);
        $config['total_rows'] = $this->Captainmodel->count_listing($config["per_page"], $data['page'], $data['searchFor'], $data['orderField'], $data['orderDirection']);
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $this->load->view('backoffice/Common/header',$data);
        $this->load->view('backoffice/Common/sidebar');
        $this->load->view('backoffice/Captain/listing',$data);
    ...
    function viewUser(){
      ...
      $data['page_title'] = 'Captain Detail';
      $pid = base64_decode(urldecode($id));
      $data['resultArr']=$this->Captainmodel->viewUser($pid); 
      $this->load->view('backoffice/Common/header',$data);
        $this->load->view('backoffice/Common/sidebar');
        $this->load->view('backoffice/Captain/viewUser',$data);
        $this->load->view('backoffice/Common/footer');
    }

    //Change Quesh status 
    function changeStatus(){
        $result = $this->Captainmodel->changeStatus();    
    }
