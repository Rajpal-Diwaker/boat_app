                                    ...
                                    <table class="table table-striped table-hover table-fw-widget dataTable" id="table1" role="grid" aria-describedby="table1_info">
                                        ...
                                        foreach ($resultArr as $key => $value) {
                                          ...
                                          <tr>
                                              ...
                                              <td class="allbutton"><?php if ($value['verified']==='false') { ?>
                                                <a class="btn-fill-md text-red border-red" onclick="changeStatus(<?php echo $value['captain_id'].','.'1';?>)">No</a>
                                                <?php } else { ?>
                                                <a class="btn-fill-md text-dark-pastel-green border-dark-pastel-green" onclick="changeStatus(<?php echo $value['captain_id'].','.'0';?>)">Yes</a>
                                                <?php } ?>
                                              </td>
                                   
                                              <td class="allbutton">
                                                <a class="btn-fill-md text-dodger-blue border-dodger-blue" href="<?php echo(ADMINURL.'Captain/viewUser/').urlencode(base64_encode($value['captain_id'])); ?>"> View</a>
                                              </td>
                                          </tr>
                                      <?php }
                                      ?>
...
function changeStatus(post_id,type){
    ...
    var type=type;
    if(type == 1){
      var showtext = 'Do you really want to Verify this Account?';
    }else{
      var showtext = 'Do you really want to De-activate this Account?';
    }
   var box =  bootbox.confirm(showtext, function(result) {
       if(result){
         ...
         $.ajax({
           url: '<?php echo(ADMINURL.'Captain/changeStatus'); ?>',
           type: 'POST',
