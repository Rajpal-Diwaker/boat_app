<style type="text/css">
  .profilepic{max-width: 75px;border-radius: 50%;}
</style>
      <div class="be-content">
        <div class="row">
          ...
          <div class="col-md-6 col-sm-6 col-xs-12">
            <a style="float: right;margin: 30px;" class="btn btn-sm btn-primary" href="<?php echo(ADMINURL.'Admin/addAdmin'); ?>"><i class="mdi mdi-plus"></i> Add New Admin</a>
          </div>
        </div>
            ...
            <div class="col-sm-12">
              <div class="card card-table">
                <div class="card-body">
                        ...
                        <div class="row be-datatable-header">
                            ...
                            <div class="col-sm-6">
                                <div id="table1_filter" class="dataTables_filter">
                                    <label>Search:<input type="text" nme="searchFor" class="form-control form-control-sm" placeholder="Search By Name" id="searchKey" onchange="sendRequest();"></label>
                                </div>
                            </div>
                        </div>
                        <div class="row be-datatable-body table-responsive">
                            <div class="col-sm-12">
                                <table class="table table-striped table-hover table-fw-widget dataTable" id="table1" role="grid" aria-describedby="table1_info">
                                  <thead>
                                    <tr>
                                      <th><span>S/N</span></th>
                                      <th>Profile Pic</th>
                                      <th data-action="sort" data-title="fullname" data-direction="ASC"><span>Full Name</span> <i class="glyphicon glyphicon-triangle-bottom"></i></th>
                                      ...
                                      <?php 
                                        foreach ($adminArr as $key => $value) {
                                          ...
                                          <tr>
                                              ...
                                              <td class="display_flex"><img src="<?php echo $value['profile_pic']; ?>" style="max-width:80px">
                                              </td>
                                              <?php }else{  ?>
                                              <td><img src="<?php echo ADMIN.'/img/person.jpeg'; ?>" style="max-width:60px">
                                              </td>
                                              <?php } ?>
                                              ...
                                              <td  style="text-align:center;" class="allbutton"><?php if ($value['status']==='inactive') { ?>
                                                <a class="btn btn-danger" onclick="changeStatus(<?php echo $value['admin_id'].','.'1';?>)">Active</a>
                                                <?php } else { ?>
                                                <a class="btn btn-info" onclick="changeStatus(<?php echo $value['admin_id'].','.'0';?>)">Inactive</a>
                                                <?php } ?>
                                              </td>
                                          </tr>
                                      <?php }
                  ...
                  <?php  }else{ echo  'No Blog Admin Added.'; } ?>
                </div>
              </div>
            </div>
          </div>
 ...
 <script src="https://cdnjs.cloudflare.com/ajax/libs/bootbox.js/4.2.0/bootbox.min.js" type="text/javascript"></script>
  <script type="text/javascript">
    var sendRequest = function(){
      ...
      window.location.href = '<?=base_url('backoffice/Admin/admin_management')?>?query='+searchKey+'&limitRows='+limitRows+'&orderField='+curOrderField+'&orderDirection='+curOrderDirection;
    }
...
function changeStatus(post_id,type){
    ...
    var type=type;
    if(type == 1){
      var showtext = 'Do you really want to Activate this Admin?';
    }else{
      var showtext = 'Do you really want to De-activate this Admin?';
    }
   var box =  bootbox.confirm(showtext, function(result) {
       if(result){
         ...
         $.ajax({
           url: '<?php echo(ADMINURL.'Admin/changeuserStatus'); ?>',
           type: 'POST',
